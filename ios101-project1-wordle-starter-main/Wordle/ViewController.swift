...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    keyboardController = KeyboardController(collectionView: keyboardCollectionView)
      ...
      keyboardController.didSelectString = { [weak self] key in
          guard let self = self else { return }
          
          if key == DELETE_KEY {
            self.boardController.deleteLastCharacter()
          } else {
              self.boardController.enter(key)
          }
        }
      /*
     //Done Exercise 3: Assign a closure to the `didSelectString` property of `keyboardController` (see KeyboardController.swift):
